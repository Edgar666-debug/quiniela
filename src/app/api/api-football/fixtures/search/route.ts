...
import { searchFixtures, ApiFootballQuotaError } from "@/lib/api-football";
import { quotaErrorMessage } from "@/lib/api-football-plan";
...
export async function GET(req: Request) {
  ...
  const q = parsed.data;
  const hasEntity = q.league !== undefined || q.team !== undefined;
  ...
  if (hasEntity && q.season === undefined) {
    return NextResponse.json({ error: "Season is required when filtering by league or team." }, { status: 400 });
  }
  if (!hasDate && !hasRange) {
  ...
  try {
    ...
    fixtures = await cache.getOrSet(key, 30_000, () => searchFixtures(q));
  } catch (err) {
    if (err instanceof ApiFootballQuotaError) {
      return NextResponse.json({ error: quotaErrorMessage(err) }, { status: 429 });
    }
